...
export const getParamData = async (id) => {
  ...
  const fechaData = await getDataString(
    `SELECT dFeEmiDE FROM tmpFactuDE_D WHERE idMov = ${id}`
  );
  ...
  const emailData = await getDataString(
    `SELECT rtrim(dEmailRec) FROM  tmpFactuDE_D3 WHERE idMov = ${id}`
  ); 
  ...
  const tipoCondicionData = await getDataString(
    `SELECT iCondOpe FROM tmpFactuDE_E7 WHERE idMov = ${id}`
  )
  ...
  return {
    ...
    cliente: {
      contribuyente: contribuyenteData || null,

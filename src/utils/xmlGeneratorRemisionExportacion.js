...
export const getParamsRemisionExportacion = async (id) => {
  // Ejecuta cada consulta y guarda el resultado
  const rucResult = await getDataString(
    `SELECT rtrim(dRucEm) + '-' + ltrim(dDVEmi) as ruc FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
  const nombreFantasiaResult = await getDataString(
    `SELECT dNomFanEmi FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
  const actividadesEconomicasCodeResult = await getDataString(
    `SELECT cActEco from tmpFactuDE_D21 WHERE tipo = 8 and idMov = ${id}`
  );
  const actividadesEconomicasDescripcionResult = await getDataString(
    `SELECT dDEsACtEco from tmpFactuDE_D21 WHERE tipo = 8 and idMov = ${id}`
  );
  const timbradoNumeroResult = await getDataString(
    `SELECT dRucEm FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
  const timbradoFechaResult = await getDataString(
    `SELECT dFeIniT FROM tmpFactuDE_C WHERE tipo = 8 and idMov = ${id}`
  );
  const tipoContribuyenteResult = await getDataString(
    `SELECT iTipCont FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
  const tipoRegimenResult = await getDataString(
    `SELECT cTipReg FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
  const codigoResult = await getDataString(
    `SELECT dEst FROM tmpFactuDE_C WHERE tipo = 8 and idMov = ${id}`
  );
  const direccionResult = await getDataString(
    `SELECT dDirEmi FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
  const numeroCasaResult = await getDataString(
    `SELECT dNumCas FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
  const departamentoResult = await getDataString(
    `SELECT cDepEmi FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
  // Aquí se corrigió el await
  const departamentoDescripcionResult = await getDataString(
    `SELECT dDesDepEmi FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
  const distritoResult = await getDataString(
    `SELECT cDisEmi FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
  const distritoDescripcionResult = await getDataString(
    `SELECT dDesDisEmi FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
  const ciudadResult = await getDataString(
    `SELECT cCiuEmi FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
  const ciudadDescripcionResult = await getDataString(
    `SELECT dDesCiuEmi FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
  const telefonoResult = await getDataString(
    `SELECT dTelEmi FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
  const emailResult = await getDataString(
    `SELECT dEmailE FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
  const denominacionResult = await getDataString(
    `SELECT dDenSuc FROM tmpFactuDE_D2 WHERE tipo = 8 and idMov = ${id}`
  );
...
export const getDataRemisionExportacion = async (id) => {
  // Ejecuta cada consulta y guarda el resultado
  const tipoDocumentoData = await getDataString(
    `SELECT iTiDE FROM tmpFactuDE_C WHERE tipo = 8 and idMov = ${id}`
  );
  const establecimientoData = await getDataString(
    `SELECT dEst from tmpFactuDE_C WHERE tipo = 8 and idMov = ${id}`
  );
  const codigoSeguridadAleatorioData = await getDataString(
    `SELECT dCodSeg from tmpFactuDE_B WHERE tipo = 8 and idMov = ${id}`
  );
  const puntoData = await getDataString(
    `SELECT dPunExp FROM tmpFactuDE_C WHERE tipo = 8 and idMov = ${id}`
  );
  const numeroData = await getDataString(
    `SELECT dNumDoc FROM tmpFactuDE_C WHERE tipo = 8 and idMov = ${id}`
  );
  const fechaData = await getDataString(
    `SELECT dFeEmiDE FROM tmpFactuDE_D WHERE tipo = 8 and idMov = ${id}`
  );
  const tipoEmisionData = await getDataString(
    `SELECT iTipEmi FROM tmpFactuDE_B WHERE tipo = 8 and idMov = ${id}`
  );
  const tipoTransaccionData = await getDataString(
    `SELECT iTipTra FROM tmpFactuDE_D1 WHERE tipo = 8 and idMov = ${id}`
  );
  const tipoImpuestoData = await getDataString(
    `SELECT iTImp FROM tmpFactuDE_D1 WHERE tipo = 8 and idMov = ${id}`
  );
  const monedaData = await getDataString(
    `SELECT cMoneOpe FROM tmpFactuDE_D1 WHERE tipo = 8 and idMov = ${id}`
  );
  const condicionAnticipoData = await getDataString(
    `SELECT iCondAnt FROM tmpFactuDE_D1 WHERE tipo = 8 and idMov = ${id}`
  );
  const condicionTipoCambioData = await getDataString(
    `SELECT dCondTiCam FROM tmpFactuDE_D1 WHERE tipo = 8 and idMov = ${id}`
  );
  const cambioData = await getDataString(
    `SELECT dTiCam FROM tmpFactuDE_D1 WHERE tipo = 8 and idMov = ${id}`
  );
  ...
  const contribuyenteData = await getDataBoolean(
    `SELECT (case
     ...
     FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  )
  const rucData = await getDataString(
    `SELECT rtrim(dRucRec) + '-' + ltrim(dDVRec) as rucCliente FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  );
  const razonSocialData = await getDataString(
    `SELECT dNomRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  );
  const nombreFantasiaData = await getDataString(
    `SELECT dNomFanRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  );
  const tipoOperacionData = await getDataString(
    `SELECT iTiOpe FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  );
  const direccionData = await getDataString(
    `SELECT dDirRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  );
  const numeroCasaData = await getDataString(
    `SELECT dNumCasRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  );
  const departamentoData = await getDataString(
    `SELECT cDepRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  ); 
  const departamentoDescripcionData = await getDataString(
    `SELECT dDesDepRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  ); 
  const distritoData = await getDataString(
    `SELECT cDisRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  ); 
  const distritoDescripcionData = await getDataString(
    `SELECT dDesDisRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  ); 
  const ciudadData = await  getDataString(
    `SELECT cCiuRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  ); 
  const ciudadDescripcionData = await getDataString(
    `SELECT dDesCiuRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  ); 
  const paisData = await getDataString(
    `SELECT cPaisRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  ); 
  const paisDescripcionData = await getDataString(
    `SELECT dDesPaisRe FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  ); 
  const tipoContribuyenteData = await getDataString(
    `SELECT iTiContRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  )
  const documentoTipoData = await getDataString(
    `SELECT iTipIDRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  ); 
  const documentoNumeroData = await getDataString(
    `SELECT dNumIDRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  ); 
  const telefonoData = await getDataString(
    `SELECT dTelRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  ); 
  const celularData = await getDataString(
    `SELECT dCelRec FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  ); 
  const emailData = await getDataString(
    `SELECT rtrim(dEmailRec) FROM  tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  ); 
  const codigoData = await getDataString(
    `SELECT dCodCliente FROM tmpFactuDE_D3 WHERE tipo = 8 and idMov = ${id}`
  ); 
  // DATOS REMISION
  const motivoData = await getDataString(
    `SELECT iMotEmiNR FROM tmpFactuDE_E6 WHERE tipo = 8 and idMov = ${id}`
  )
  const tipoResponsableData = await getDataString(
    `SELECT iRespEmiNR FROM tmpFactuDE_E6 WHERE tipo = 8 and idMov = ${id}`
  )
  const kilometrosData = await getDataString(
    `SELECT dKmR FROM tmpFactuDE_E6 WHERE tipo = 8 and idMov = ${id}`
  )
  const fechaFacturaData = await getDataString(
    `SELECT dFecEm FROM tmpFactuDE_E6 WHERE tipo = 8 and idMov = ${id}`
  )
  //detalles - items
  const codigoItemData = await getDataString(
    `SELECT dCodInt FROM tmpFactuDE_E8 WHERE tipo = 8 and idMov = ${id}`
  );
  const descripcionItemData = await getDataString(
    `SELECT dDesProSer FROM tmpFactuDE_E8 WHERE tipo = 8 and idMov = ${id}`
  );
  const unidadMedidaItemData = await getDataString(
    `SELECT cUniMed FROM tmpFactuDE_E8 WHERE tipo = 8 and idMov = ${id}`
  );
  const cantidadItemData = await getDataString(
    `SELECT dCantProSer FROM tmpFactuDE_E8 WHERE tipo = 8 and idMov = ${id}`
  );
  const precioUnitarioItemData = await getDataString(
    `SELECT dPUniProSer FROM tmpFactuDE_E81 WHERE tipo = 8 and idMov = ${id}`
  );
  const ivaTipoItemData = await getDataString(
    `SELECT iAfecIVA FROM tmpFactuDE_E82 WHERE tipo = 8 and idMov = ${id}`
  );
  const ivaBaseItemData = await getDataString(
    `SELECT dPropIVA FROM tmpFactuDE_E82 WHERE tipo = 8 and idMov = ${id}`
  );
  const ivaItemData = await getDataString(
    `SELECT dTasaIVA FROM tmpFactuDE_E82 WHERE tipo = 8 and idMov = ${id}`
  );
  // detalle transporte
  const tipoTransporteData = await getDataString(
    `SELECT iTipTrans FROM tmpFactuDE_E10 WHERE tipo = 8 and idMov = ${id}`
  );
  const modalidadTranporteData = await getDataString(
    `SELECT iModTrans FROM tmpFactuDE_E10 WHERE tipo = 8 and idMov = ${id}`
  );
  const tipoResponsableTranspeData = await getDataString(
    `SELECT iRespFlete FROM tmpFactuDE_E10 WHERE tipo = 8 and idMov = ${id}`
  );
  const inicioEstimadoTransladoData = await getDataString(
    `SELECT dIniTras FROM tmpFactuDE_E10 WHERE tipo = 8 and idMov = ${id}`
  );
  const finEstimadoTransladoData = await getDataString(
    `SELECT dFinTras FROM tmpFactuDE_E10 WHERE tipo = 8 and idMov = ${id}`
  );
  const paisDestinoData = await getDataString(
    `SELECT cPaisDest FROM tmpFactuDE_E10 WHERE tipo = 8 and idMov = ${id}`
  );
  const paisDestinoNombre  = await getDataString(
    `SELECT dDesPaisDest FROM tmpFactuDE_E10 WHERE tipo = 8 and idMov = ${id}`
  );
  // datos de la salida
  const direccionSalidaData = await getDataString(
    `SELECT dDirLocSal FROM tmpFactuDE_E101 WHERE tipo = 8 and idMov = ${id}`
  );
  const numeroCasaSalidaData = await getDataString(
    `SELECT dNumCasSal FROM tmpFactuDE_E101 WHERE tipo = 8 and idMov = ${id}`
  );
  const complementoDireccion1SalidaData = await getDataString(
    `SELECT dComp1Sal FROM tmpFactuDE_E101 WHERE tipo = 8 and idMov = ${id}`
  );
  const complementoDireccion2SalidaData = await getDataString(
    `SELECT dComp2Sal FROM tmpFactuDE_E101 WHERE tipo = 8 and idMov = ${id}`
  );
  const departamentoSalidaData = await getDataString(
    `SELECT cDepSal FROM tmpFactuDE_E101 WHERE tipo = 8 and idMov = ${id}`
  );
  const departamentoDescripcionSalidaData = await getDataString(
    `SELECT dDesDepSal FROM tmpFactuDE_E101 WHERE tipo = 8 and idMov = ${id}`
  );
  const distritoSalidaData = await getDataString(
    `SELECT cDisSal FROM tmpFactuDE_E101 WHERE tipo = 8 and idMov = ${id}`
  );
  const distritoDescripcionSalidaData = await getDataString(
    `SELECT dDesDisSal FROM tmpFactuDE_E101 WHERE tipo = 8 and idMov = ${id}`
  );
  const ciudadSalidaData = await getDataString(
    `SELECT cCiuSal FROM tmpFactuDE_E101 WHERE tipo = 8 and idMov = ${id}`
  );
  const ciudadDescripcionSalidaData = await getDataString(
    `SELECT dDesCiuSal FROM tmpFactuDE_E101 WHERE tipo = 8 and idMov = ${id}`
  );
  // datos de la Entrega
  const direccionEntregaData = await getDataString(
    `SELECT dDirLocEnt FROM tmpFactuDE_E102 WHERE tipo = 8 and idMov = ${id}`
  );
  const numeroCasaEntregaData = await getDataString(
    `SELECT dNumCasEnt FROM tmpFactuDE_E102 WHERE tipo = 8 and idMov = ${id}`
  );
  const complementoDireccion1EntregaData = await getDataString(
    `SELECT dComp1Ent FROM tmpFactuDE_E102 WHERE tipo = 8 and idMov = ${id}`
  );
  const complementoDireccion2EntregaData = await getDataString(
    `SELECT dComp2Ent FROM tmpFactuDE_E102 WHERE tipo = 8 and idMov = ${id}`
  );
  const departamentoEntregaData = await getDataString(
    `SELECT cDepEnt FROM tmpFactuDE_E102 WHERE tipo = 8 and idMov = ${id}`
  );
  const departamentoDescripcionEntregaData = await getDataString(
    `SELECT dDesDepEnt FROM tmpFactuDE_E102 WHERE tipo = 8 and idMov = ${id}`
  );
  const distritoEntregaData = await getDataString(
    `SELECT cDisEnt FROM tmpFactuDE_E102 WHERE tipo = 8 and idMov = ${id}`
  );
  const distritoDescripcionEntregaData = await getDataString(
    `SELECT dDesDisEnt FROM tmpFactuDE_E102 WHERE tipo = 8 and idMov = ${id}`
  );
  const ciudadEntregaData = await getDataString(
    `SELECT cCiuEnt FROM tmpFactuDE_E102 WHERE tipo = 8 and idMov = ${id}`
  );
  const ciudadDescripcionEntregaData = await getDataString(
    `SELECT dDesCiuEnt FROM tmpFactuDE_E102 WHERE tipo = 8 and idMov = ${id}`
  );
  const telefonoContactoEntregaData = await getDataString(
    `SELECT dTelEnt FROM tmpFactuDE_E102 WHERE tipo = 8 and idMov = ${id}`
  );

  // datos del vehiculo
  const tipoVehiculoData = await getDataString(
    `SELECT dTiVehTras FROM tmpFactuDE_E103 WHERE tipo = 8 and idMov = ${id}`
  );
  const marcaVehiculoData = await getDataString(
    `SELECT dMarVeh FROM tmpFactuDE_E103 WHERE tipo = 8 and idMov = ${id}`
  );
  const documentoTipoVehiculoData = await getDataString(
    `SELECT dTipIdenVeh FROM tmpFactuDE_E103 WHERE tipo = 8 and idMov = ${id}`
  );
  const documentoNumeroVehiculoData = await getDataString(
    `SELECT dNroIDVeh FROM tmpFactuDE_E103 WHERE tipo = 8 and idMov = ${id}`
  );
  const observacionVehiculoData = await getDataString(
    `SELECT dAdicVeh FROM tmpFactuDE_E103 WHERE tipo = 8 and idMov = ${id}`
  );
  const numeroMatriculaVehiculoData = await getDataString(
    `SELECT dNroMatVeh FROM tmpFactuDE_E103 WHERE tipo = 8 and idMov = ${id}`
  );
  const numeroVueloVehiculoData = await getDataString(
    `SELECT dNroVuelo FROM tmpFactuDE_E103 WHERE tipo = 8 and idMov = ${id}`
  );
  ...
  const contribuyenteTransportistaData = await getDataBoolean(
    `SELECT (case
     ...
     FROM tmpFactuDE_E104 WHERE tipo = 8 and idMov = ${id}`
  );
  const nombreTransportistaData = await getDataString(
    `SELECT dNomTrans FROM tmpFactuDE_E104 WHERE tipo = 8 and idMov = ${id}`
  );
  const rucTransportistaData = await getDataString(
    `SELECT rtrim(dRucTrans) + '-' + ltrim(dDVTrans) as rucTransportista FROM tmpFactuDE_E104 WHERE tipo = 8 and idMov = ${id}`
  );
  const documentoTipoTransportistaData = await getDataString(
    `SELECT iTipIDTrans FROM tmpFactuDE_E104 WHERE tipo = 8 and idMov = ${id}`
  );
  const documentoNumeroTransportistaData = await getDataString(
    `SELECT dNumIDTrans  FROM tmpFactuDE_E104 WHERE tipo = 8 and idMov = ${id}`
  );

  // datos del chofer
  const documentoNumeroChofer = await getDataString(
    `SELECT dNumIDChof FROM tmpFactuDE_E104 WHERE tipo = 8 and idMov = ${id}`
  );
  const nombreChoferData = await getDataString(
    `SELECT dNomChof FROM tmpFactuDE_E104 WHERE tipo = 8 and idMov = ${id}`
  );
  const direccionChoferData = await getDataString(
    `SELECT dDirChof FROM tmpFactuDE_E104 WHERE tipo = 8 and idMov = ${id}`
  );
...
const dataRemision = async (id) => {
    const remision = await getDataRemisionExportacion(id);

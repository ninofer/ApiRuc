...
export const getDataRemision = async (id) => {
  ...
  const inicioEstimadoTransladoData = await getDataString(
    `SELECT dIniTras FROM tmpFactuDE_E10 WHERE tipo = 1 and idMov = ${id}`
  );
  const finEstimadoTransladoData = await getDataString(
    `SELECT dFinTras FROM tmpFactuDE_E10 WHERE tipo = 1 and idMov = ${id}`
  );
  const paisDestinoData = await getDataString(
    `SELECT cPaisDest FROM tmpFactuDE_E10 WHERE tipo = 1 and idMov = ${id}`
  );
  const paisDestinoNombre  = await getDataString(
    `SELECT dDesPaisDest FROM tmpFactuDE_E10 WHERE tipo = 1 and idMov = ${id}`
  );
  // datos de la salida
  const direccionSalidaData = await getDataString(
    `SELECT dDirLocSal FROM tmpFactuDE_E101 WHERE tipo = 1 and idMov = ${id}`
  );
  const numeroCasaSalidaData = await getDataString(
    `SELECT dNumCasSal FROM tmpFactuDE_E101 WHERE tipo = 1 and idMov = ${id}`
  );
  const complementoDireccion1SalidaData = await getDataString(
    `SELECT dComp1Sal FROM tmpFactuDE_E101 WHERE tipo = 1 and idMov = ${id}`
  );
  const complementoDireccion2SalidaData = await getDataString(
    `SELECT dComp2Sal FROM tmpFactuDE_E101 WHERE tipo = 1 and idMov = ${id}`
  );
  const departamentoSalidaData = await getDataString(
    `SELECT cDepSal FROM tmpFactuDE_E101 WHERE tipo = 1 and idMov = ${id}`
  );
  const departamentoDescripcionSalidaData = await getDataString(
    `SELECT dDesDepSal FROM tmpFactuDE_E101 WHERE tipo = 1 and idMov = ${id}`
  );
  const distritoSalidaData = await getDataString(
    `SELECT cDisSal FROM tmpFactuDE_E101 WHERE tipo = 1 and idMov = ${id}`
  );
  const distritoDescripcionSalidaData = await getDataString(
    `SELECT dDesDisSal FROM tmpFactuDE_E101 WHERE tipo = 1 and idMov = ${id}`
  );
  const ciudadSalidaData = await getDataString(
    `SELECT cCiuSal FROM tmpFactuDE_E101 WHERE tipo = 1 and idMov = ${id}`
  );
  const ciudadDescripcionSalidaData = await getDataString(
    `SELECT dDesCiuSal FROM tmpFactuDE_E101 WHERE tipo = 1 and idMov = ${id}`
  );
  // datos de la Entrega
  const direccionEntregaData = await getDataString(
    `SELECT dDirLocEnt FROM tmpFactuDE_E102 WHERE tipo = 1 and idMov = ${id}`
  );
  const numeroCasaEntregaData = await getDataString(
    `SELECT dNumCasEnt FROM tmpFactuDE_E102 WHERE tipo = 1 and idMov = ${id}`
  );
  const complementoDireccion1EntregaData = await getDataString(
    `SELECT dComp1Ent FROM tmpFactuDE_E102 WHERE tipo = 1 and idMov = ${id}`
  );
  const complementoDireccion2EntregaData = await getDataString(
    `SELECT dComp2Ent FROM tmpFactuDE_E102 WHERE tipo = 1 and idMov = ${id}`
  );
  const departamentoEntregaData = await getDataString(
    `SELECT cDepEnt FROM tmpFactuDE_E102 WHERE tipo = 1 and idMov = ${id}`
  );
  const departamentoDescripcionEntregaData = await getDataString(
    `SELECT dDesDepEnt FROM tmpFactuDE_E102 WHERE tipo = 1 and idMov = ${id}`
  );
  const distritoEntregaData = await getDataString(
    `SELECT cDisEnt FROM tmpFactuDE_E102 WHERE tipo = 1 and idMov = ${id}`
  );
  const distritoDescripcionEntregaData = await getDataString(
    `SELECT dDesDisEnt FROM tmpFactuDE_E102 WHERE tipo = 1 and idMov = ${id}`
  );
  const ciudadEntregaData = await getDataString(
    `SELECT cCiuEnt FROM tmpFactuDE_E102 WHERE tipo = 1 and idMov = ${id}`
  );
  const ciudadDescripcionEntregaData = await getDataString(
    `SELECT dDesCiuEnt FROM tmpFactuDE_E102 WHERE tipo = 1 and idMov = ${id}`
  );
  const telefonoContactoEntregaData = await getDataString(
    `SELECT dTelEnt FROM tmpFactuDE_E102 WHERE tipo = 1 and idMov = ${id}`
  );

  // datos del vehiculo
  const tipoVehiculoData = await getDataString(
    `SELECT dTiVehTras FROM tmpFactuDE_E103 WHERE tipo = 1 and idMov = ${id}`
  );
  const marcaVehiculoData = await getDataString(
    `SELECT dMarVeh FROM tmpFactuDE_E103 WHERE tipo = 1 and idMov = ${id}`
  );
  const documentoTipoVehiculoData = await getDataString(
    `SELECT dTipIdenVeh FROM tmpFactuDE_E103 WHERE tipo = 1 and idMov = ${id}`
  );
  const documentoNumeroVehiculoData = await getDataString(
    `SELECT dNroIDVeh FROM tmpFactuDE_E103 WHERE tipo = 1 and idMov = ${id}`
  );
  const observacionVehiculoData = await getDataString(
    `SELECT dAdicVeh FROM tmpFactuDE_E103 WHERE tipo = 1 and idMov = ${id}`
  );
  const numeroMatriculaVehiculoData = await getDataString(
    `SELECT dNroMatVeh FROM tmpFactuDE_E103 WHERE tipo = 1 and idMov = ${id}`
  );
  const numeroVueloVehiculoData = await getDataString(
    `SELECT dNroVuelo FROM tmpFactuDE_E103 WHERE tipo = 1 and idMov = ${id}`
  );
  ...
  const contribuyenteTransportistaData = await getDataBoolean(
    `SELECT (case
     ...
     FROM tmpFactuDE_E104 WHERE tipo = 1 and idMov = ${id}`
  );
  const nombreTransportistaData = await getDataString(
    `SELECT dNomTrans FROM tmpFactuDE_E104 WHERE tipo = 1 and idMov = ${id}`
  );
  const rucTransportistaData = await getDataString(
    `SELECT rtrim(dRucTrans) + '-' + ltrim(dDVTrans) as rucTransportista FROM tmpFactuDE_E104 WHERE tipo = 1 and idMov = ${id}`
  );
  const documentoTipoTransportistaData = await getDataString(
    `SELECT iTipIDTrans FROM tmpFactuDE_E104 WHERE tipo = 1 and idMov = ${id}`
  );
  const documentoNumeroTransportistaData = await getDataString(
    `SELECT dNumIDTrans  FROM tmpFactuDE_E104 WHERE tipo = 1 and idMov = ${id}`
  );

  // datos del chofer
  const documentoNumeroChofer = await getDataString(
    `SELECT dNumIDChof FROM tmpFactuDE_E104 WHERE tipo = 1 and idMov = ${id}`
  );
  const nombreChoferData = await getDataString(
    `SELECT dNomChof FROM tmpFactuDE_E104 WHERE tipo = 1 and idMov = ${id}`
  );
  const direccionChoferData = await getDataString(
    `SELECT dDirChof FROM tmpFactuDE_E104 WHERE tipo = 1 and idMov = ${id}`
  );
  ...
  return {
    ...
    descripcion: descripcionData[0]?.dDes || null,
    observacion: observacionData[0]?.dObs || null,
    ...
    descuentoGlobal: 0, // Valor fijo según ejemplo
    anticipoGlobal: 0,  // Valor fijo según ejemplo
    ...
    cliente: {
      ...
      numeroCasa: numeroCasaData[0]?.dNumCasRec || null,
      departamento: departamentoData[0]?.cDepRec || null,
      departamentoDescripcion: departamentoDescripcionData[0]?.dDesDepRec || null,
    ...
    detalleTransporte: {
      ...
      salida: {
        ...
        numeroCasa: numeroCasaSalidaData[0]?.dNumCasSal || null,
      ...
      entrega: {
        ...
        numeroCasa: numeroCasaEntregaData[0]?.dNumCasEnt || null,
      ...
      vehiculo: {
        ...
        obs: obsVehiculoData[0]?.dAdicVeh || null,
      ...
      transportista: {
        ...
        chofer: {
          documentoNumero: documentoNumeroChoferData[0]?.dNumIDChof || null,
